...
$(document).ready(function() {
    // Flag to track the state (true if full name is displayed, false otherwise)
    var isFullDisplayed = false;

    // Event handler for clicking on campaign name column
    $('table').on('click', '.campaign-name-column', function() {
        // Get the current cell and its contents
        var cell = $(this);
        var span = cell.find('span');
        var fullName = span.data('fullname');
        
        // Toggle between full and truncated text
        if (isFullDisplayed) {
            // Show truncated text
            span.text(fullName.substring(0, 10));
        } else {
            // Show full text
            span.text(fullName);
        }
...
var summary_table= $('#summary_data-table').DataTable( {
    ...
    render: function(data, type, full, meta) {
		...
		{data:'average_call_hold',name:'average_call_hold', className: "text-left"},
		{data:'total_calls',name:'total_calls', className: "text-center"},
		...
		{data:'success_percentage',name:'success_percentage', className: "text-center"},
                <!-- {data:'1_10_secs',name:'1_10_secs', className: "text-center text_nowrap"}, -->
{
    data: '1_10_secs',
    name: '1_10_secs',
    ...
    render: function(data, type, full, meta) {
        ...
        // Split the cleaned data into count and percentage
        var parts = cleanedData.split(' ');
        var count = parts[0];
        var percentage = parts[1];

        // Format the output with count and percentage side by side
        var output = '<span>' + count + '</span>';
        output += '<span style="margin-left: 15px; color: blue;">' + percentage + '</span>';
  ...
  ajax: 
	...
	[ 
        ...
        {data:'total_failure',name:'total_failure', className: "text-center"},
          ...
          @endif
          { "data": 'remarks',name: 'remarks',className: "text-center word_break",render: function(data, type, row) {
          ...
          {data:'calldates',name:'calldates', className: "text-center word_break"},
	  	    {data:'startdates', name:'startdates', className: "text-center word_break",render: function(data, type, row){
            return (data !== null && data.trim() !== '') ? data : '-'; // Display '-' for null or empty strings
          }
          },
	  	    {data:'completedates', name:'completedates', className: "text-center word_break",render: function(data, type, row){
            return (data !== null && data.trim() !== '') ? data : '-'; // Display '-' for null or empty strings
          }
          },
  ],
      ...
      var campaign_table = $('#context_list-table').DataTable({
        ...
        columns: 
	      [
        	  ...
        	  { data: 'user_name', name: 'user_name', className: 'text-left', orderable: true, searchable: true },
    		    @endif
          	{ data: 'campaign_type', name: 'campaign_type', className: 'td_nowrap text-left', render: function(data, type, row) {
            if (data === 'C') {
