...
//            var context = document.getElementById("context_value").value;
  //          var contextError = document.getElementById("context_error");
        ...
        var customised_radio = document.getElementById("customised");
        var non_customised_radio = document.getElementById("non_customised");

        if (customised_radio.checked) {
            var campaign_type = customised_radio.value;
        } else if (non_customised_radio.checked) {
            var campaign_type = non_customised_radio.value;
        }
        ...
        if ((campaign_type == 'N' && fileInput.files.length === 0) || remarks === "" || company === "" || location === "" || language === "" || type === "") {

		
        if (campaign_type == 'N' && fileInput.files.length === 0) {
                    fileError.textContent = "Prompt File is required.";
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const uploadAudioFileInput = document.getElementById('upload_file');
        const uploadAudioFileLabel = document.querySelector('label[for="upload_audio_file"]');
        const fileErrorSpan = document.getElementById('file_error');
        const fileFormatSpan = document.querySelector('.text-sky-500');

        // Function to toggle visibility of the upload audio file input and label
        function toggleUploadAudioFile() {
            if (customisedRadio.checked) {
                uploadAudioFileInput.style.display = 'none'; // Hide the input
                uploadAudioFileLabel.style.display = 'none'; // Hide the label
                fileErrorSpan.style.display = 'none'; // Hide the error message span
                fileFormatSpan.style.display = 'none'; // Hide the file format span
            } else {
                uploadAudioFileInput.style.display = 'block'; // Show the input
                uploadAudioFileLabel.style.display = 'block'; // Show the label
                fileErrorSpan.style.display = 'block'; // Show the error message span
                fileFormatSpan.style.display = 'block'; // Show the file format span
            }
        }

        // Add event listeners to listen for changes in the radio buttons
        customisedRadio.addEventListener('change', toggleUploadAudioFile);
        nonCustomisedRadio.addEventListener('change', toggleUploadAudioFile);

        // Initially check the state of the radio buttons on page load
        toggleUploadAudioFile();
    });
</script>
...
//     // Check if the selected radio button is "No"
//     if ($(this).val() === "N") {
//         // Clear the fields when switching to "No"
//         clearFormFields();
//         formData = {}; // Reset the stored values
        
//     } else {
//         // If switching to "Yes", restore previous values if they exist
//         if (Object.keys(formData).length === 0) {
           

//             // If formData is empty, save the current values
//             formData = {
...
//                 type: $('#type').val(),
//                 context_value: $('#context_value').val(),
//                 remarks: $('#remarks').val()
//             };
...
//             $('#upload_file').val(formData.upload_file);
//             $('#company_name').val(formData.company_name);
//             $('#location').val(formData.location);
//             $('#language_code').val(formData.language_code);
//             $('#type').val(formData.type);
//             $('#context_value').val(formData.context_value);
...
$(document).ready(function() {
    ...
    $('input[name="campaign_type"]').on('change', function() {
        ...
        if ($(this).val() === "N") {
            ...
            // If switching to "Yes", save the current values before clearing
            if (Object.keys(formData).length === 0) {
                // If there's no saved data, save the current values into formData
                formData = {
                    upload_file: $('#upload_file').val(),
                    company_name: $('#company_name').val(),
                    location: $('#location').val(),
            ...
            // Clear the fields before restoring the saved values
            clearFormFields();
...
<div class="w-full bg-white shadow-md rounded-lg px-8 pt-6 pb-8 mb-4 flex flex-col" style="border-radius:30px;">
    <div class="px-3" style="background-color: #FFF; text-align: center; color: black; height: 50px; padding-top: 2px; margin-bottom: 6px; margin-right:300px;">
      <!--  <h2 class="text-2xl font-medium" style="font-weight: bold;">Create Prompt</h2> -->
	<span class="mx-2 text-black text-xl uppercase font-bold"> Create Prompt </span>
    </div>
    ...
    <label class="text-lg uppercase font-medium text-gray-800 block mb-2 dark:text-gray-300" 
           for="user_avatar" 
           style="text-align: right; line-height: 42px; font-size: medium; ">
        Customised OBD 
        <span data-toggle="tooltip" data-original-title="Select campaign type">
       ...
       <!--Prompt File Upload and Date -->
            <div class="-mx-3 flex mb-6" style="margin-right:400px !important;">
                <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                    <label class="text-lg uppercase font-medium text-gray-800 block mb-2 dark:text-gray-300" for="upload_audio_file" style="text-align: right; line-height: 42px; font-size: medium;">
                    Upload prompt<span data-toggle="tooltip" data-original-title="Upload audio file only mp3 or wav format">
            <label style="color: #FF0000;">*</label> [?]
        </span> 
                    </label>
                </div>
                <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                    <input class="w-full bg-gray-200 text-black border border-gray-200 rounded py-2 px-3 mb-1" 
                        aria-describedby="user_avatar_help" id="upload_file" type="file" name="upload_file" 
                        title="Upload MP3 / WAV file" accept="audio/mpeg, audio/wav" style="width: 500px;"
                        data-toggle="tooltip" data-placement="top" data-html="true" title="" autocomplete="off">

                        <div id="file_error" class="error-message"></div>
                <span class="text-sm text-sky-500">The file must be MP3 or Wav.</span>
                </div>
            </div>
	...
	<!--Prompt File Upload and Date -->
       <!-- <div class="- mx-3 flex mb-6">
	    <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                <label class=" text-lg uppercase font-medium text-gray-800 block mb-2 dark:text-gray-300" for="user_avatar" style="text-align: right; line-height: 42px; font-size: medium;">Upload prompt<span style="color: #ff0000">*</span></label>
		</div>
		<div class="md:w-1/2 px-3 mb-6 md:mb-0">
		  <input class="w-64 bg-gray-200 text-black border border-gray-200 rounded py-2 px-3 mb-1" aria-describedby="user_avatar_help" id="upload_file" style="width: 500px;" autocomplete="off"
	...
	<div class="-   mx-3 md:flex mb-6" style="margin-right:400px !important;">
        <!-- Company Name input -->
           <div class="md:w-1/2 px-3 mb-6 md:mb-0" style="text-align: right; line-height: 42px;">
                <label class=" text-lg uppercase font-medium text-gray-800 block mb-2 dark:text-gray-300" for="user_avatar" style="text-align: right; line-height: 42px; font-size: medium; ">
                    Company Name<span data-toggle="tooltip" data-original-title="Enter Company Name">
            <label style="color: #FF0000;">*</label> [?]
        </span> 
                </label>
        </div>
        <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                ...
                <input name="company_name" autocomplete="off"
                    ...
                    type="text" onkeydown="return /[a-z, ]/i.test(event.key)" placeholder="Company name/ Person/ Institute" minlength = "3" maxlength="10" style="width: 500px;">
	...
	<!-- Location -->
            ...
            <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                <label class=" text-lg uppercase font-medium text-gray-800 block mb-2 dark:text-gray-300" for="user_avatar" style="text-align: right; line-height: 42px; font-size: medium;">
                    Location<span data-toggle="tooltip" data-original-title="Select Location">
            <label style="color: #FF0000;">*</label> [?]
        </span> 
                </label>
                </div>
                <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                        ...
                        <select name="location" id="location" class="w-full bg-gray-200 text-black border border-gray-200 rounded py-2 px-3 mb-1" style="width: 500px; float: left" autocomplete="off">
	...
	<!-- Language Code -->
            ...
            <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                <label class=" text-lg uppercase font-medium text-gray-800 block mb-2 dark:text-gray-300" for="user_avatar" style="text-align: right; line-height: 42px; font-size: medium;">
                    Language Code<span data-toggle="tooltip" data-original-title="Select Language">
            <label style="color: #FF0000;">*</label> [?]
        </span> 
                </label>
                </div>
                <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                        <select name="language_code" id="language_code" class="w-full bg-gray-200 text-black border border-gray-200 rounded py-2 px-3 mb-1" style="width: 500px; float: left" autocomplete="off">
	...
	<div class="- mx-3 flex mb-6" style="margin-right:400px !important;">
           ...
           <div class="md:w-1/2 px-3 mb-6 md:mb-0">
			<label class=" text-lg uppercase font-medium text-gray-800 block mb-2 dark:text-gray-300" for="user_avatar" style="text-align: right; line-height: 42px; font-size: medium;">
                    Type<span data-toggle="tooltip" data-original-title="Select Campaign Type">
            <label style="color: #FF0000;">*</label> [?]
        </span> 
                </label>
                        </div>
                <div class="md:w-1/2 px-3 mb-6 md:mb-0">
				<select name="type" id="type" class="w-full bg-gray-200 text-black border border-gray-200 rounded py-2 px-3 mb-1" style="width: 500px; float: left" autocomplete="off">
	...
	<!-- Context input -->
           <div class="md:w-1/2 px-3 mb-6 md:mb-0" style="text-align: right; line-height: 42px;">
                <label class=" text-lg uppercase font-medium text-gray-800 block mb-2 dark:text-gray-300" for="user_avatar" style="text-align: right; line-height: 42px; font-size: medium;">
                    Context<span data-toggle="tooltip" data-original-title="Context name">
            <label style="color: #FF0000;">*</label> [?]
        </span> 
                </label>
	</div>
		...
		<input type="hidden" name="context" id="context" value="">
                 <input name="context_value" autocomplete="off"
                    class="w-64 bg-gray-200 text-black border border-gray-200 rounded py-2 px-3 mb-1" id="context_value" title="context" readonly
                     minlength = "5" maxlength="15" style="width: 500px;">  
	...
	<div class="-   mx-3 md:flex mb-6" style="margin-right:400px !important;">
        <!-- Remarks input -->
           <div class="md:w-1/2 px-3 mb-6 md:mb-0" style="text-align: right; line-height: 42px;">
                <label class=" text-lg uppercase font-medium text-gray-800 block mb-2 dark:text-gray-300" for="user_avatar" style="text-align: right; line-height: 42px; font-size: medium;">
                    Remarks<span data-toggle="tooltip" data-original-title="Enter remarks">
            <label style="color: #FF0000;">*</label> [?]
        </span> 
                </label>
        </div>
        <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                ...
                <textarea name="remarks"
                    ...
                    type="text" placeholder="Maximum 50 characters" maxlength = "50" style="width: 500px; height:100px;" autocomplete="off"></textarea>
...
<!-- Buttons (Clear, Submit, Cancel) -->
<div class="mx-3 md:flex md:space-x-4 mb-6" style="width: 900px;margin-left: 500px !important;">
    <!-- Clear button -->
    <div class="md:w-1/3 px-3 mb-4 md:mb-0">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const uploadAudioFileInput = document.getElementById('upload_file');
        const fileAudioError = document.getElementById('file_type_error');

        uploadAudioFileInput.addEventListener('change', function(event) {
            const selectedFile = event.target.files[0];
            const allowedTypes = ['audio/mpeg', 'audio/wav'];

            if (!allowedTypes.includes(selectedFile.type)) {
                fileAudioError.textContent = 'Invalid file. Please upload a valid MP3 or WAV file.';
                fileAudioError.style.color = "red";
                // Clear the file input value to allow selecting a valid file
                event.target.value = '';
            } else {
                fileAudioError.innerText = ''; // Clear any previous error message
            }
        });
    });
</script>
...
function clearInputs() {
    ...
    document.getElementById("context_value").value = '';
    document.getElementById("remarks").value = '';
    
    // You can clear other inputs as needed
    // For example, if you want to clear dropdowns
    document.getElementById("location").selectedIndex = 0; // Select the first option
    document.getElementById("language_code").selectedIndex = 0; // Select the first option
    document.getElementById("type").selectedIndex = 0; // Select the first option
