...
<script>
    ...
    lastThreeDaysData.forEach(data => {
        ...
        aggregatedData[chartLabels].busy += parseInt(data.total_busy);
        aggregatedData[chartLabels].noAnswer += parseInt(data.total_no_answer);
    });
    ...
    // Bar chart rendering (initial chart showing only Total Calls)
    ...
    let barChart = new Chart(ctx, {
        ...
        options: {
            ...
            onClick: function (event) {
                const activeElements = [aggregatedData[chartLabels].success += parseInt(data.total_success);
        aggregatedData[chartLabels].busy += parseInt(data.total_busy);
        aggregatedData[chartLabels].noAnswer += parseInt(data.total_no_answer);]

                if (activeElements.length > 0) {
                    const clickedElement = activeElements[0];
                    const index = clickedElement.index; // Get the clicked index
                    const selectedDate = dates[index]; // Get the corresponding date

                    // Call the drilldown function to show detailed bars for Success, Busy, and No Answer
                    loadDetailedData(selectedDate);
                }
            },
        },
    });

    // Drilldown function to load more detailed data for the selected date
    function loadDetailedData(selectedDate) {
        const selectedData = aggregatedData[selectedDate]; // Get aggregated data for the selected date

        if (selectedData) {
            // Destroy the existing chart before creating a new one
            barChart.destroy();

            // Create a new chart to display the breakdown of Success, Busy, and No Answer
            barChart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [
                        {
                            label: `Calls on ${selectedDate}`,
                            data: [selectedData.success, selectedData.busy, selectedData.noAnswer],
                            backgroundColor: [
