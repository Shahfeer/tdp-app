...
$sql_calls = "SELECT * FROM calls where DATE_FORMAT(schedule_at, 'Y-m-d H:i:s') >= DATE_FORMAT(NOW(), 'Y-m-d H:i:s') and call_status in ('N', 'P') limit 1";
print($sql_calls);
...
if ($qur_calls->num_rows > 0) {
	...
	print("sudo Copy ".$time_interval." ".$file_count." ".$input_file_path.$context." ".$output_file_path);
	$cns = shell_exec("sudo Copy ".$time_interval." ".$file_count." ".$input_file_path.$context." ".$output_file_path);
} 
