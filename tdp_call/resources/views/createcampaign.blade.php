        ...
        $.ajax({
            ...
            data: fd,
                ...
                error: function (xhr, status, error) 
                {
                    ...
                    var errorElement = document.getElementById('error_message');
                    errorElement.textContent = "Error: Please upload a valid  file!";
                    errorElement.style.color = 'red';
                    fileInput.value = '';
...
function validateForm() {
    ...
    if (fileInput.value === "") {
        fileerror.textContent = "CSV File is required.";
        fileerror.style.color = "red";
        flag = false;
    }
    ...
    if (retrytimeInput.value === "" && retrycountInput.value != 0) {
        retrytimeerror.textContent = "Retry Time Interval is required.";
        retrytimeerror.style.color = "red";
        flag = false;
    } else {
...
<div class="container full-width">
    <div class="px-3" style="background-color: #FFF; text-align: center; color: black; height: 50px; padding-top: 8px; margin-bottom: 20px;margin-right:300px;">
    ...
    <form action="{{ route('file-import') }}" method="POST" name="campaign_form" id="campaign_form" onsubmit="return validateForm()" enctype="multipart/form-data">
        ...
        <div class="- mx-4 flex mb-6" style="margin-right:400px !important;">
            <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                <label class=" text-lg uppercase font-medium text-gray-800 block mb-2 dark:text-gray-300" for="user_avatar" style="text-align: right; line-height: 42px; font-size: medium;">Customised OBD<span data-toggle="tooltip" data-original-title="Select campaign type">
        ...
        <div class="- mx-3 flex mb-6" style="margin-right:400px !important;">
            <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                <label class=" text-lg uppercase font-medium text-gray-800 block mb-2 dark:text-gray-300" for="user_avatar" style="text-align: right; line-height: 42px; font-size: medium;">Upload file<span data-toggle="tooltip" data-original-title="Choose csv format only">
            <label style="color: #FF0000;">*</label> [?]
        </span>
	          </div>
    ...
    <div class="flex items-center">
        <input
            class="bg-gray-200 text-black border border-gray-200 rounded py-2 px-3"
            accept=".csv"
            id="upload_file"
            type="file"
            onclick="chooseFile()"
            ...
            <a id="download_sample_link" href="#" download
            class="bg-gray-500 text-white py-2 px-4 border-b-4 hover:border-b-2 border-gray-2000 hover:border-gray-100 rounded-full"
            style="width: auto; position:absolute;text-align: center; margin-left:400px !important;">
            Download Sample
        </a>
    </div>
    <div id="file_error" class="error-message"></div>
	      ...
	      <div class="- mx-3 flex mb-6" style="margin-right:400px !important;">
            ...
            <div class="md:w-1/2 px-3 mb-6 md:mb-0">
        		  <label class=" text-lg uppercase font-medium text-gray-800 block mb-2 dark:text-gray-300" for="user_avatar" style="text-align: right; line-height: 42px; font-size: medium;">Call Retry Count<span data-toggle="tooltip" data-original-title="Choose retry count">
            <label style="color: #FF0000;">*</label> [?]
        </span>
			      </div>
		        <div class="md:w-1/2 px-3 mb-6 md:mb-0">
        		    <select
            			class="w-full bg-gray-200 text-black border border-gray-200 rounded py-2 px-3 mb-1" style="width: 350px;" autocomplete="off"
            			id="retry_count"
            			name="retry_count"
            			title="Retry Count" >
	      ...
	      <div class="md:w-1/2 px-3" style="margin-right:400px !important;" >
        ...
        <div class="-   mx-3 md:flex mb-6" style="margin-right:400px !important;">
            ...
            <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                <label class=" text-lg uppercase font-medium text-gray-800 block mb-2 dark:text-gray-300" for="user_avatar" style="text-align: right; line-height: 42px; font-size: medium;">
                    Retry Time Interval<span data-toggle="tooltip" data-original-title="Set time interval">
            <label style="color: #FF0000;">*</label> [?]
        </span>
                </label>
		        </div>
		        <div class="md:w-1/2 px-3 mb-6 md:mb-0"> 
                <input type="text" name="retry_time" id="retry_time" class="w-full bg-gray-200 text-black border border-gray-200 rounded py-2 px-3 mb-1" style="width: 350px; float: left" autocomplete="off" placeholder="Min. 900 Secs & Max. 3600 Secs" onkeydown="return /^[0-9]$/.test(event.key) || ['Backspace', 'Delete', 'ArrowLeft', 'ArrowRight'].includes(event.key)" >
                    <br><br>
	    ...
	    <!-- Context -->
        <div class="-mx-3 md:flex mb-6 context_msg" style="margin-right:400px !important;" >
	        <!-- Context input -->
            <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                <label class=" text-lg uppercase font-medium text-gray-800 block mb-2 dark:text-gray-300" for="user_avatar" style="text-align: right; line-height: 42px; font-size: medium;">
            ...
            <input type="hidden" name="ivr_id" id="ivr_id" value="">
			    <select name="context" id="context" class="w-full bg-gray-200 text-black border border-gray-200 rounded py-2 px-3 mb-1" style="width: 350px; float: left;margin-left:10px;" autocomplete="off">
        			<option value="" disabled selected>Select a Context</option>
        			<!-- You can use JavaScript to populate the options dynamically -->
    			</select>
                <button id="playButton" type="button" class=" md:w-full bg-gray-900 text-white py-2 px-4 border-b-4 hover:border-b-2 border-gray-600 hover:border-gray-100 rounded-full" style="margin-left: 10px; width: 100px; display: none; float: left;position:absolute; background-color:lawngreen !important; color:black;">
                        Play
                </button><br><br>
		        <div id="context_error" class="error-message" style="margin-left: 11px;"></div>
        ...
        <audio id="audioPlayer" controls style="display: none"></audio>
          ...
          <div class="mx-3 md:flex md:space-x-4 mb-6" style="width: 600px;margin-left: 500px !important;">
	          <!-- Clear button -->
