...
@section('content')
        ...
        @if ($displayContent)
            <div>
                ...
                <div class="card-header" style="background-color:#fff;margin-top:0.2%;">
                    <div class="px-3"
                        style="background-color: #FFF; height: 30px; padding-top: 8px; margin-bottom: 10px;">
                            ...
                            @if (date('Y-m-d', strtotime($admin->call_entry_time)) == now()->toDateString())
                                ...
                                <div class="w-full md:w-1/3 px-2 mb-3" style="width: 33%;">
                                    <div class="relative w-full"
                                        style="
                @if ($admin->call_status == 'O') background-color: #9fdf9f; /* green color */
                ...
                @elseif($admin->call_status == 'D')
                    background-color: #ff8080; /* Light red color */ @endif">
                                        @php
    ...
    ?>
    <div class="bg-white shadow-md rounded-lg px-8 pt-6 pb-8 mb-4 flex flex-col" style="height:700px">
   ...
   @if (Auth::user()->user_master_id != 1)
        <div class="card" style="margin-top:10px">
            <div class="px-3 flex flex-wrap"
                style="background-color: #FFF; height: 20px; padding-top: 10px; margin-bottom: 20px;">
                <div style="width:60%">
                    <h2 class="text-2xl font-bold">Last 3 Day's Campaign</h2>
                </div>
                <div style="width:40%">
                    <h2 style=" text-align: center" class="text-2xl font-bold">Last one Week's Campaign</h2>
                </div>
            </div>
            ...
            <div class="flex flex-wrap" style="align-items:center">
                ...
                <div class="w-full" style="width:40%;vertical-align:center">
                    <center>
        ...
        <script>
            ...
            lastThreeDaysData.forEach(data => {
                ...
                aggregatedData[chartLabels].failure += parseInt(data.total_failure) + parseInt(data.total_busy) +
                    parseInt(data.total_no_answer);
            });
            ...
            const lastThreeDates = Array.from({
                length: 3
            }, (_, i) => {
                const date = new Date();
            ...
            function updateChart(yAxisScale) {
                ...
                console.log(maxDataValue);
                const numTicks = Math.ceil(maxDataValue /
                    yAxisScale); // Calculate number of ticks based on maxDataValue and yAxisScale
                console.log(numTicks);
            ...
            if (totalCallsCount !== 0 || totalSuccessCount !== 0 || totalFailureCount !== 0 || totalBusyCount != 0 ||
