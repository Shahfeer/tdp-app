...
<div class="modal fade bs-example-modal-md" id="recordCountModal" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="noChannelsModal" aria-hidden="true" data-backdrop="false" style=" position:fixed; left: 50%; top: 50%; transform: translate(-50%, -50%); overflow: visible; padding-right: 15px; width: 400px;">
  <div class="modal-dialog" >
    <div class="modal-content" style="box-shadow: 0px 4px 8px rgba(0, 0, 0, 0.2);">
      ...
      <button type="button" id="downloadButton" class=" md:w-full bg-gray-900  text-white py-2 px-4 border-b-4 hover:border-b-2 border-gray-500 hover:border-gray-100 rounded-full" data-dismiss="modal">Download</button>
      <button type="button" class=" md:w-full bg-gray-900  text-white py-2 px-4 border-b-4 hover:border-b-2 border-gray-500 hover:border-gray-100 rounded-full button0" data-dismiss="modal" onclick = "window.location.reload();">Cancel</button>
...
<div class="preloader-wrapper_1" style="display:none;">
      ...
      <div class="text" style="color: white; background-color:#f27878; padding: 10px; margin-left:600px; margin-top:150px;">
	...
	<style>
.overflow_split { overflow-wrap: anywhere; min-width: 125px; }   
.preloader-wrapper_1 {
      display: flex;
...
function checkDataCountAndDisplayModal() {
    ...
    if (totalRecordCount > 10000) {
        ...
        $('#detail_data-table tbody').html('<tr><td colspan="12" class="center-text">No data available</td></tr>');
...
var summary_table= $('#summary_data-table').DataTable( {
	...
	[
        ...
        columns:
              [
                        { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false, className: "text-center"},
                {data:'campaign_date',name:'campaign_date', className: "text-center text_nowrap", width: "150px"},
                ...
                {data:'11_15_secs',name:'11_15_secs', className: "text-center text_nowrap"},
                {data:'16_20_secs',name:'16_20_secs', className: "text-center text_nowrap"},
                {data:'21_25_secs',name:'21_25_secs', className: "text-center text_nowrap"},
                {data:'26_30_secs',name:'26_30_secs', className: "text-center text_nowrap"},
...
var summary_report_table= $('#summary_report_data-table').DataTable( {
        ...
        ajax: 
	{
        ...
        [
                ...
                {data:'total_busy',name:'total_busy', className: "text-center"},
                {data:'total_no_answer',name:'total_no_answer', className: "text-center"},
		{data:'failed',name:'failed', className: "text-center"}, 
               <!-- {data:'failed',name:'failed', className: "text-center"}, -->
...
var campaign_table = $('#campaign_list-table').DataTable( 
{
  ...
  ajax: 
	...
	[ 
		    ...
		    { data: 'campaign_name', name: 'campaign_name', className: 'text-left overflow_split' },
        { data: 'ivr_id', name: 'ivr_id', className: 'text-left' },
        { data: 'context', name: 'context', className: 'text-left overflow_split' },
        { data: 'total_calls', name: 'total_calls', className: 'text-center'},
	      { data:'total_success',name:'total_success', className: "text-center"}, 
        {data:'total_failure',name:'total_failure', className: "text-center"},
          ...
          @if(Auth::user()->user_master_id === 1)
            {data: 'action', name: 'action', orderable: false, searchable: false, className: "text-center word_break", exportOptions: { columns: ":visible" }},
            {data: 'neron_id', name: 'neron_id', orderable: false, searchable: false, className: "text-center word_break", exportOptions: { columns: ":visible" },
              "render": function (data, type, row, meta) 
              {
                if (type === 'display') 
                {
                    ...
                    {
                        return '<span class="tooltip-span" data-neron-id="' + data + '">' + data + '</span>';
                    } 
                    else 
                    {
          ...
          @endif
          { "data": 'remarks',name: 'remarks',className: "text-center word_break overflow_split",render: function(data, type, row) {
            return (data !== null && data.trim() !== '') ? data : '-'; // Display '-' for null or empty strings
          }
          },
          {data:'calldates',name:'calldates', className: "text-center overflow_split"},
	  	    {data:'startdates', name:'startdates', className: "text-center overflow_split",render: function(data, type, row){
            return (data !== null && data.trim() !== '') ? data : '-'; // Display '-' for null or empty strings
          }
          },
	  	    {data:'completedates', name:'completedates', className: "text-center overflow_split",render: function(data, type, row){
            return (data !== null && data.trim() !== '') ? data : '-'; // Display '-' for null or empty strings
          }
          },
  ],
  ...
  function fetchNeronDetails(neronId, element) 
  {
      ...
      $.ajax({
          url: '/neron_details',
          method: 'GET',
          data: { neron_id: neronId },
          success: function(response) 
          {
              // Assuming the response contains the neron_details
              var neronDetails = response.neron_details;

              var detailsHTML = generateDetailsHTML(neronDetails); // Generate HTML for details
...
function getStatusLabel(status)
{
    	switch (status)
      {
            ...
            case 'S':
                 	return '<span class="text-danger">Campaign Stopped</span>';
           	default:
...
$(function () {
      var credit_table = $('#credit_table').DataTable({
        ...
        columns: 
	      [
          	{ data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false, className: "text-center" },
          	{ data: 'user_name', name: 'user_name', orderable: false, className: "text-left" },
...
$(function () {
	...
	[
          	...
          	{ data: 'campaign_name', name: 'campaign_name', className: 'text-left' },
            { data: 'ivr_id', name: 'ivr_id', className: 'text-center'},
          	{ data: 'context', name: 'context', className: 'td_nowrap text-left' },
	  	...
	  	{ data: 'call_entry_time', name: 'call_entry_time', className: 'text-center'},
          	{data: 'action', name: 'action', orderable: false, searchable: false,className: 'text-center', exportOptions: { columns: ":visible" }},
        ],
        rowCallback: function (row, data) {
      ...
      var campaign_table = $('#context_list-table').DataTable({
        ...
        columns: 
	      [
        	  ...
        	  { data: 'user_name', name: 'user_name', className: 'text-left', orderable: true, searchable: true },
    		    @endif
          	{ data: 'ivr_id', name: 'ivr_id', className: 'td_nowrap text-left' },
		        { data: 'context', name: 'context', className: 'td_nowrap text-left' },
            { data: 'remarks', name: 'remarks', className: 'td_nowrap text-left' },
            {data:'prompt',name:'prompt', className: "text-left"}, 
            {data:'prompt_status',name:'prompt_status', className: "text-center",render: function(data, type, row) {
            if (data === 'Y') {
...
$(function () 
{
      ...
      ({
            ...
            columns: 
	          [
                { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false, className: "text-center" },
                { data: 'board_name', name: 'board_name', orderable: false, searchable: false, className: "text-left"},
                { data: 'server_id', name: 'server_id', orderable: false, searchable: false, className: "text-left", 
                  render: function(data, type, row) 
                  {
                    return '<a href="#" class="server-details" data-server-id="' + row.server_id + '">' + data + '</a>';
                  } 
                },
                { data: 'ip_address', name: 'ip_address', className: 'text-center' },
                { data: 'neron_status', name: 'neron_status', className: 'text-center',
                  render: function (data, type, row) 
                  {
                    if (data === 'Y') 
                    {
                ...
                { data: 'running_status', name: 'running_status', className: 'td_nowrap text-center', 
                  "render": function (data, type, row, meta) 
                  {
                      if (type === 'display') 
                      {
                          ...
                          {
                            //console.log(data);
                              return '<span class="tooltip-campaign" data-campaign-id="' + data + '">' + data + '</span>';
                          } 
                          else 
                          {
                              return ''; // Display an empty string if neron_id is null, undefined, or empty
                ...
                { data: 'neron_con_time', name: 'neron_con_time', className: 'td_nowrap text-center' },
                {data: 'action', name: 'action', orderable: false, searchable: false, exportOptions: { columns: ":visible" }},
            ],
            rowCallback: function (row, data) 
            {
      ...
      $(document).on('mouseenter', '.tooltip-campaign', function() 
      {
          ...
          var runningStatus = $(this).data('campaign-id'); // Assuming running status is available as data attribute
      ...
      $('#gsm_table tbody').on('click', 'a.server-details', function(e) 
      {
          ...
          $.ajax({
              ...
              success: function(response) 
              {
                  ...
                  var channelStatusData = response.channel_status; // Assuming 'channel_status' is an array

                  var modalContent = '<table class="table">';
                  modalContent += '<thead><tr><th>Channel</th><th>SIM Number</th><th>Status</th></tr></thead>';
                  modalContent += '<tbody>';
                  ...
                  for (var i = 0; i < channelStatusData.length; i++) 
                  {
                      ...
                      {
                          modalContent += '<td style="color: green;">' + channelStatusData[i].status + '</td>';
                      } 
                      else 
                      {
                          modalContent += '<td style="color: red;">' + channelStatusData[i].status + '</td>';
                      }
        ...
        columns: 
	      [
            ...
            {data:'prompt',name:'prompt', className: "text-left"}, 
          	{data:'entry_time',name:'entry_time', className: "text-center"},
          	{data: 'action', name: 'action', orderable: false, searchable: false, exportOptions: { columns: ":visible" }},
            {data: 'approve', name: 'approve', orderable: false, searchable: false, exportOptions: { columns: ":visible" }},
        ],
        rowCallback: function (row, data) {
