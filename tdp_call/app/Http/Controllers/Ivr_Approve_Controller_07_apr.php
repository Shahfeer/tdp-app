        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->addColumn('action', function ($query) {
                    $playBtn = '<a href="javascript:void(0)"  class= "play-pause" title="Play/Stop the Audio" style="width: 40px; height: 40px; color: #000;" id="audioid_'.$query->prompt_id.'" onclick="playAudio(\'' . $query->prompt . '\', \''.$query->prompt_id.'\')"><i class="fas fa-play"></i></a>';

                    $downloadBtn = '<a title="Download the Audio" href="storage/app/user_prompt_files/' . $query->prompt . '" style="width: 40px; height: 40px; color: #000;" download><svg viewBox="0 0 20 20" width="32" height="24" fill="#000000" xmlns="http://www.w3.org/2000/svg"><path d="M13 8V2H7v6H2l8 8 8-8h-5zM0 18h20v2H0v-2z"/></svg></a>';
                ...
                ->addColumn('approve', function ($query) {
                    ...
                    $approveBtn = '<a href="javascript:void(0)" class=" md:w-full bg-gray-900 text-white py-2 px-4 border-b-4 hover:border-b-2 border-gray-500 hover:border-gray-100 rounded-full" onclick="select_sender(\'' . $query->prompt_id . '\',)">Approve</a>';

                    $declineBtn = '<a href="javascript:void(0)" class="md:w-full bg-gray-900 text-white py-2 px-4 border-b-4 hover:border-b-2 border-gray-500 hover:border-gray-100 rounded-full" onclick="decline_ivr(\'' . $query->prompt_id . '\')">Decline</a>';
